...
public partial class project : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        sql = sql + "AppImageHeading, AppImageCaption, AppWebLinksTitle, AppSuccessStoryHeading, TransPendingID From APPROVED_PROJ_PAGES Where ApprovedID=@ApprovedID and Visible=1";
        ...
        while (dr.Read())
        {
            ...
            connCountRefs.Open();
            string sqlCountRefs = "Select count(PendingID) as NumberOfRefs from RESOURCE_LINKS where PendingID=@PendingID";
            SqlCommand cmdCountRefs = new SqlCommand(sqlCountRefs, connCountRefs);
            int TransPendingID = Convert.ToInt32(dr["TransPendingID"]);
            cmdCountRefs.Parameters.Add("@PendingID", TransPendingID);
            ...
            while (drCountRefs.Read())
            {
                ...
                if (count > 0)
                {
                    ...
                    connRefs.Open();
                    string sqlRefs = "Select ResourceLinkUrl, ResourceLinkText from RESOURCE_LINKS where PendingID=@PendingID";
                    SqlCommand cmdRefs = new SqlCommand(sqlRefs, connRefs);
                    cmdRefs.Parameters.Add("@PendingID", TransPendingID);
                    SqlDataReader drRefs = cmdRefs.ExecuteReader();
                    while (drRefs.Read())
                    {
                        string LinkText = drRefs["ResourceLinkText"].ToString(); string LinkUrl = drRefs["ResourceLinkUrl"].ToString();
            ...
            if (dr["AppSuccessStory"].ToString() != "")
            {
                ...
                string successURL = ConfigurationManager.AppSettings.Get("URLRoot").ToString() + successLink;
